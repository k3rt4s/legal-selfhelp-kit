...
6. For each genuine change: fix the claim and its row if a primary source turns up in one attempt. Otherwise cut the claim to that file's gap list and add a CHANGELOG correction line naming it, so a reader who acted on the old text can see that it moved.
