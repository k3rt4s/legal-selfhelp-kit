...
- A backreference row inherits the source it names, not the URL that happens to be nearest. `scripts/check_sources.py` reads a named host first, then the literal word "above" as adjacency, then a section or rule number such as 25-222, and only then a descriptor such as "Chapter 9" matched as whole words against the earlier rows and their URL paths. The number outranks the words because a file citing many sections of one code repeats the same words on every row. This matters because interleaving a one-off source between an anchor row and the rows referring back to it used to send every one of them to the wrong page, which reads in the report as a claim that has gone missing. "Backreference names X, but no earlier row in this file cites it" is a real citation defect: the row is pointing at something the file does not carry.
