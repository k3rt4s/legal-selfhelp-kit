...
- A backreference row inherits the source it names, not the URL that happens to be nearest. `scripts/check_sources.py` reads a named host first, then the literal word "above" as adjacency, then a descriptor such as "Chapter 9" or a case name matched as whole words against the earlier rows and their URL paths, and only then, for a row offering no usable descriptor, a section or rule number such as 25-222. The descriptor outranks the number because a row usually names a document and then locates a passage inside it: "Same Owens v. Purcel opinion, quoting R.C. 2305.117(B)" cites the opinion, not the statute. Words that only locate a passage, page and comment and paragraph among them, are stopwords and name nothing, and a descriptor is matched on whole words because Michigan's rules book and the order adopting one rule both carry "mrpc" somewhere in a path and only one carries it as a word. A word shorter than four letters is not read as a descriptor at all, so a row naming only an initialism falls through to the number or to the row above. This matters because interleaving a one-off source between an anchor row and the rows referring back to it used to send every one of them to the wrong page, which reads in the report as a claim that has gone missing. "Backreference names X, but no earlier row in this file cites it" is a real citation defect: the row is pointing at something the file does not carry.
- A source cell can mention "above" and still name a specific document. Plain "same as above" remains adjacency, but a cell that names a host, filename, document descriptor, or section gets resolved by that name before adjacency is used as a fallback. That fixed the August 2026 pattern where rows such as "Fee Arbitration Program Rules PDF, same URL as above" were tested against neighboring pages instead of the document they named. Still read the row's own source cell before acting on a claim-missing verdict.
...
- A row that names its source too loosely gives the resolver nothing to work with, and the URL it gets is a best effort rather than a citation. "law.justia.com, section pages cited above" named a host that carries thousands of pages, and a 2026-08-27 backreference recheck found it resolving Indiana row 47 to the wrong statute entirely (IC 34-11-2-11, a limitations section, not either small-claims jurisdictional-limit section it was supposed to cite). Fixed 2026-09-07 (D2): Indiana rows 47 and 48 and Mississippi row 63 now name each section's own law.justia.com page explicitly in the Source cell instead of pointing at "rows above."
- A reasoned-by-elimination row is a genuine inference over two or more sources, not a citation, and no amount of resolver work turns it into one: there is no single page behind it to name. Fixed 2026-09-07 and 2026-09-08: Mississippi rows 64 and 65 and Utah row 72 now say they are this kit's inference rather than pretending to cite one document, so `parse_reference_file` correctly reports them as UNPARSEABLE_ROW ("no URL found in row") instead of silently mis-resolving them. This is a wording fix, not a code fix: the checker's resolver cannot and should not be taught to turn an inference into a citation, so any future row of this shape must be worded the same way rather than left to trigger backreference resolution by accident.
