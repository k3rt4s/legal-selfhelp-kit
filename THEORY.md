...
- The word "above" in a source cell beats the document the cell names. Adjacency is tried before descriptor matching, so "Fee Arbitration Program Rules PDF, same URL as above" resolves to whatever row sits directly above it. That is why the August 2026 sweep reported twenty rows as no longer carrying their claim when a hand re-check cleared almost all of them: the checker had tested them against pages they do not cite. Never act on a claim-missing verdict without reading the row's own source cell first. `FUTURE_FEATURES.md` item 5 is the fix.
...
- A row that names its source too loosely gives the resolver nothing to work with, and the URL it gets is a best effort rather than a citation. "law.justia.com, section pages cited above" names a host that carries thousands of pages; "Sections 15-1-29 and 15-1-49 themselves (Confirmed rows above), reasoned by elimination" is an inference over two sources and has no single page behind it. Rows like these read as confirmed and are not, and no amount of resolver work fixes one: the cell has to name the document. Indiana rows 47 and 48 and Mississippi rows 63 to 65 are the known instances.
