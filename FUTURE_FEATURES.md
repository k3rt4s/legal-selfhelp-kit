...
- **sizecap** Stop the URL extractor swallowing a closing backtick, and raise the cap for large archives. Maintenance item 2.
  `score: kind=bug gain=0.2/0.5/1.5 p=1 freq=2 hours=0.15/0.3/0.6 ai=2 rev=two-way conf=measured id=sizecap`
  `return: likelihood 1 in 1 per sweep, about 2 sweeps a year, so about twice a year, from the 4 rows in the August 2026 sweep whose fetched URL carried a trailing backtick, three in verification_mn.md and one in verification_nj.md, counted by re-reading the chunked source_health reports of 2026-08-22 on 2026-09-06; impact 4 rows fetch a URL the row does not cite and land in HTTP_ERROR or UNREACHABLE, so they are triaged by hand every sweep and their claims go unjudged, 0.2 to 1.5 h per sweep; evidence URL_RE at scripts/check_sources.py line 52, which excludes whitespace and closing brackets but not a backtick or a comma, the four captured URLs in the chunked source_health reports of 2026-08-22, and MAINTAINING.md step 4, which attributes the New Jersey failure to the 5 MB cap when the sweep report shows an HTTP 404 on a URL ending in a backtick and a comma, so the cap has never been observed to fire and that note needs correcting`
